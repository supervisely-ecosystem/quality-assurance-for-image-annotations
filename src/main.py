...
def main_func(user_id: int, team: TeamInfo, workspace: WorkspaceInfo, project: ProjectInfo):
    ...
    if g.api.file.dir_exists(team.id, tf_project_dir):
        ...
        # Check for heatmaps - but only force recalc if status_ok doesn't exist
        # This prevents unnecessary recalculation when heatmaps are being generated by another request
        ...
        if not g.api.file.exists(team.id, heatmaps_path):
            # Check if heatmaps are currently being generated (status_ok exists)
            if g.api.file.exists(team.id, heatmaps_status_path):
                sly.logger.log(
                    g._INFO,
                    f"Heatmaps for {heatmaps.basename_stem!r} are being generated or were completed. Skipping recalculation.",
                )
            else:
                # Neither heatmaps nor status exist - need full recalculation
                force_stats_recalc = True
                sly.logger.log(
                    g._WARNING,
                    f"The calculated stat {heatmaps.basename_stem!r} does not exist. Forcing full stats recalculation...",
                )
    ...
    # Wait for background threads to complete before releasing the lock
    # This ensures heatmaps and archives are fully uploaded before next request starts
    sly.logger.log(g._INFO, "Waiting for background threads to complete...")
    thread1.join()
    thread2.join()
    sly.logger.log(g._INFO, "All background threads completed")

    # Clean up lock file after ALL processing is complete (including background threads)
    lock_file_path = f"{project_fs_dir}/.processing.lock"
    sly.fs.silent_remove(lock_file_path)
